...
const columns: (page: string) => ColumnDef<ListItem>[] = (page) => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <a
          className="text-purple-600 dark:text-purple-400 hover:underline underline-offset-4"
          href={"/repo" + row.original.familyUrl}
        >
          {row.original.family}
        </a>
      );
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Tooltip>
          <TooltipTrigger className="font-mono text-gray-600 dark:text-gray-400">
            {sha.length > 10 ? `${sha.substring(0, 10)}...` : sha}
          </TooltipTrigger>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const cates: string[] = Array.isArray(cate)
        ? cate
        : cate.split(",").filter((c) => c.trim() != "");
          ...
          <Tooltip>
            <TooltipTrigger>
              <div className="flex flex-wrap gap-1">
                {cates.slice(0, 3).map((c) => (
                  <Category key={c} cat={c} />
                ))}
                {cates.length > 3 ? (
                  <p className="mt-auto text-gray-500">...</p>
                ) : (
                  ""
                )}
              </div>
      ...
      if (day == "Invalid Date") {
        return (
          <Tooltip>
            ...
            <TooltipContent>The Build Date is not available</TooltipContent>
          </Tooltip>
        );
      }
...
function Category({ cat }: { cat: string }) {
  const cate = cat.trim();
  return (
    <div className="p-1 px-2 border border-foreground rounded-xl bg-gray-100 dark:bg-gray-800 text-sm">
